...
final class BazelClient: BazelBuildProcess {
    ...
    private let logger = Logger(label: "BazelBuild")
    ...
    func start(
            ...
            processResultsQueue.async {
                ...
                while input.hasBytesAvailable {
                    do {
                        ...
                        self.logger.trace("Received BEP event: \(event)")

                        bepHandler(event)

                        if event.lastMessage {
                            self.logger.trace("Received last BEP event")

                            bepFileHandle.closeFile()
                            bepFileHandle.readabilityHandler = nil
                        }
                    } catch {
                        self.logger.error("Failed to parse BEP event: \(error)")
                        return
                    }
                }
            }
        ...
        stdout.fileHandleForReading.readabilityHandler = { [processResultsQueue] handle in
            ...
            guard !data.isEmpty else {
                self.logger.trace("Received Bazel standard output EOF")
            ...
            processResultsQueue.async {
                self.logger.trace("Received Bazel standard output: \(data)")
                outputHandler(data)
            }
        }
        ...
        stderr.fileHandleForReading.readabilityHandler = { [processResultsQueue] handle in
            let data = handle.availableData
            guard !data.isEmpty else {
                self.logger.trace("Received Bazel standard error EOF")
            ...
            processResultsQueue.async {
                self.logger.trace("Received Bazel standard error: \(data)")
                outputHandler(data)
            }
        }
        ...
        process.terminationHandler = { process in
            self.logger.debug("xcode.sh exited with status code: \(process.terminationStatus)")
            processDispatchGroup.leave()
        }
        ...
        startedHandler(
            ...
            { [process, bepPath] finalTargetPatterns, workingDirectory, environment, scriptPath in
                ...
                self.logger.info("Starting Bazel with command: \(command)")
    ...
    func createOutputGroupsFile(_  finalTargetPatterns: String) -> String {
        let filePath: String = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent(ProcessInfo().globallyUniqueString).path
        FileManager.default.createFile(atPath: filePath, contents: Data(finalTargetPatterns.utf8))
        return filePath
    }
}
...
final class CleanBuildFolderProcess: BazelBuildProcess {
    ...
    private let logger: Logger = .init(label: "CleanBuildFolderProcess")
    ...
    func start(
        ...
        stdout.fileHandleForReading.readabilityHandler = { [processResultsQueue] handle in
            let data = handle.availableData
            guard !data.isEmpty else {
                self.logger.trace("Received CleanBuildFolder standard output EOF")
                stdout.fileHandleForReading.readabilityHandler = nil
                return
            }
            ...
            processResultsQueue.sync {
                if let output = String(data: data, encoding: .utf8) {
                    self.logger.error("Received CleanBuildFolder standard output: \(output)")
                }
            }
        }
        ...
        stderr.fileHandleForReading.readabilityHandler = { [processResultsQueue] handle in
            let data = handle.availableData
            guard !data.isEmpty else {
                self.logger.trace("Received CleanBuildFolder standard error EOF")
                stderr.fileHandleForReading.readabilityHandler = nil
                return
            }
            ...
            processResultsQueue.sync {
                if let output = String(data: data, encoding: .utf8) {
                    self.logger.error("Received CleanBuildFolder standard error: \(output)")
                }
            }
        }
...
private final class UniqueTargetsProcess {
    ...
    private let logger: Logger = .init(label: "UniqueTargetsProcess")
    ...
    func start(
        ...
        uniqueBuildLabelsHandler: @escaping (Result<[String], any Error>) -> Void
    ) {
        guard !process.isRunning else {
        ...
        stdout.fileHandleForReading.readabilityHandler = { [processResultsQueue] handle in
            let data = handle.availableData
            guard !data.isEmpty else {
                self.logger.trace("Received UniqueTargets standard output EOF")
                stdout.fileHandleForReading.readabilityHandler = nil
                processDispatchGroup.leave()
            ...
            processResultsQueue.async {
                self.logger.trace("Received UniqueTargets standard output: \(data)")
            }
        }
        ...
        stderr.fileHandleForReading.readabilityHandler = { [processResultsQueue] handle in
            let data = handle.availableData
            guard !data.isEmpty else {
                self.logger.trace("Received UniqueTargets standard error EOF")
                stderr.fileHandleForReading.readabilityHandler = nil
                processDispatchGroup.leave()
                return
            }
            ...
            processResultsQueue.async {
                self.logger.trace("Received UniqueTargets standard error: \(data)")
        ...
        processDispatchGroup.enter()
        process.terminationHandler = { process in
            self.logger.debug("UniqueTargets exited with status code: \(process.terminationStatus)")
            processDispatchGroup.leave()
        }
